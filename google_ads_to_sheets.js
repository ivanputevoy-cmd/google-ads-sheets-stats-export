 ...
 * На каждом запуске по расписанию дописывает снимок статистики ЗА ВСЁ ВРЕМЯ
 * (накопленный итог) в иерархическом виде. Старые строки не стираются — история
 * копится сверху вниз. Частоту задаёт расписание в кабинете (по умолчанию «Ежечасно»
 * = раз в час); каждый запуск результативен, холостых прогонов нет.
 *
...
// Минимальный зазор между записями, в часах. Должен совпадать с расписанием в кабинете,
// иначе будут холостые прогоны. Расписание «Ежечасно» → ставь 1 (данные раз в час).
// В Google Ads нет расписания «раз в N часов»; реже, чем раз в час, — только «Ежедневно».
// Значение чуть меньше расписания (минус 10 минут внутри) — защита от дублей и дрейфа времени.
var EVERY_N_HOURS = 1;
